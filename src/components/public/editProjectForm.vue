 ...
 * @LastEditTime: 2019-11-18 16:09:58
...
<template>
  <a-form :form="form" class="form">
    ...
    >
      <div
        class="upload"
        v-decorator="[`pic`, {rules:[{required: true, message: '请选择产品图片'}]}]"
      >
    ...
    <a-form-item>
      <a-button type="primary" html-type="submit" class="fr" @click="editProject">保存</a-button>
    </a-form-item>
  </a-form>
</template>
...
export default {
  ...
  methods: {
    ...
    // 选择图片
    selectPic(event) {
      let obj = document.getElementById("editinput");
      obj.click();
    },
    // 提交表单
    editProject(e) {
  ...
  watch: {
    editProjectfields: {
      handler() {
        this.imageUrl = this.editProjectfields.pic
        // console.log(this.editId);
        // if (this.editId !== "") {
        let nameArr = ["name", "orgPrice", "price", "pic", "brief"];
        this.form = this.$form.createForm(this, {
          mapPropsToFields: () => {
            let fieldObj = {};
            nameArr.forEach(v => {
              fieldObj[v] = this.$form.createFormField({
                value: this.editProjectfields[v]
              });
            });
            return fieldObj;
          }
